...
if [[ -n "$KAFKA_ADVERTISED_LISTENERS" ]]; then
    PORT_VALUE=$(eval "printenv $MY_METADATA_NAME")
    NEWVALUE1=${KAFKA_ADVERTISED_LISTENERS//MYPORTW/$PORT_VALUE}
#    PUBLIC_IP_VALUE=$1
#    NEWVALUE2=${NEWVALUE1//PUBLIC_IP/$PUBLIC_IP_VALUE}
    eval "export KAFKA_ADVERTISED_LISTENERS=$NEWVALUE1"
fi
if [[ -n "$KAFKA_LISTENERS" ]]; then
    PORT_VALUE=$(eval "printenv $MY_METADATA_NAME")
    NEWVALUE1=${KAFKA_LISTENERS//MYPORTW/$PORT_VALUE}
#    PUBLIC_IP_VALUE=$1
#    NEWVALUE2=${NEWVALUE1//PUBLIC_IP/$PUBLIC_IP_VALUE}
    eval "export KAFKA_LISTENERS=$NEWVALUE1"
fi
printenv

set +e
sed "s/10\.[0-9]*\.[0-9]*\.[0-9]*/& $MY_DOMAIN /" /etc/hosts > /etc/hosts2
